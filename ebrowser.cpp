...
EBrowser::EBrowser(QWidget *parent)
    : QWidget(parent), s_webView(new QWebView(this)), s_webPath(new QLabel(this)), s_question(new QLabel(this))
{
    setWindowTitle("Excel Import for OTS");
...
void EBrowser::loadFinishedLogic(bool s)
{
    if(!s)
    {
        ...
        {
            s_webView->setHtml("<center><b>An unknown error occured in loading "+s_webView->url().toString()+".</B></center>");
        }
        else
        {
